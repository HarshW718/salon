...
app.controller("cashdeskCtrl", [
  ...
  function (
          ...
          angular.forEach(response.data.data, function (single_service) {
              ...
              if (UNPAID_INVOICE.customer.coupon != undefined) {
                if (UNPAID_INVOICE.customer.coupon.length > 0) {
                  $scope.current_customer_coupon =
                    UNPAID_INVOICE.customer.coupon.filter(function (c_cc) {
            ...
            if (single_service.coupon != undefined) {
              if (single_service.coupon.cc_number != undefined) {
                single_service.label =
                  single_service.name +
                  "  ( ₹" +
                  single_service.sales_price +
                  ") ";
              } else {
                single_service.label =
                  single_service.name +
              ...
              if (single_service.coupon.cc_number != undefined) {
                if (membership_price_valid == 0) {
                  single_service.label =
                    ...
                    single_service.sales_price +
                    ") - " +
                    single_service.service_category.name;
                } else if (membership_price_valid == 1) {
                  single_service.label =
                    single_service.name +
                    "  ( ₹" +
                    single_service.membership_price +
                    ") - " +
                    single_service.service_category.name;
                }
              } else {
    ...
    $scope.list_service_female = function () {
      $http
        .get(APP.API + "list_service_female")
        .then(function (response) {
          try {
            if (response.data.status == "401") {
              restSessionOut.getRstOut();
    ...
    $scope.list_service_category = function () {
      ...
      $http
        .get(APP.API + "list_service_category_forselect")
        .then(function (response) {
          try {
            if (response.data.status == "401") {
              restSessionOut.getRstOut();
    ...
    $scope.list_product_categories = function () {
      ...
      $http
        .get(APP.API + "list_product_category")
        .then(function (response) {
          try {
            if (response.data.status == "401") {
              restSessionOut.getRstOut();
        ...
        .then(function (response) {
          ...
          if (UNPAID_INVOICE.id == undefined) {
            if (response.data.last_row != null) {
              var remove_last_char_from_prefix =
                response.data.prifix[0].prefix_title.substring(
                  0,
                  response.data.prifix[0].prefix_title.length - 1
                );

              if (
                remove_last_char_from_prefix ==
                response.data.last_row.invoice_prefix
              ) {
                $scope.invoice_number =
                  response.data.last_row.invoice_no != undefined
                    ? response.data.last_row.invoice_no + 1
                    : response.data.prifix[0].prefix_title.charAt(
                        response.data.prifix[0].prefix_title.length - 1
                      );
              } else {
                $scope.invoice_number =
            ...
            $scope.display_invoice_number =
              $scope.invoice_prefix + $scope.invoice_number;
          } else {
            $scope.display_invoice_number =
              UNPAID_INVOICE.invoice_prefix + UNPAID_INVOICE.invoice_no;
    ...
    $scope.paidAmount = function (invoiceId) {
      if (invoiceId != 0) {
        ...
        $http
          .get(APP.API + "get_payment_by_invoice_id/" + invoiceId)
          .then(function (response) {
            try {
              if (response.data.status == "401") {
                restSessionOut.getRstOut();
    ...
    $scope.worker_list = function () {
      $http
        .get(APP.API + "worker_list_for_calendar/" + 0)
        .then(function (response) {
          try {
            if (response.data.status == "401") {
              restSessionOut.getRstOut();
    ...
    $scope.invoice_data = function () {
      ...
      $scope.customer.full_name =
        UNPAID_INVOICE.customer != null
          ? UNPAID_INVOICE.customer.firstname +
            " " +
            UNPAID_INVOICE.customer.lastname
          : "";
      $scope.invoice.to_pay_id =
        UNPAID_INVOICE.to_pay_id == undefined || UNPAID_INVOICE.to_pay_id == 0
          ? $scope.first_staff_id
          : UNPAID_INVOICE.to_pay_id;
      ...
      angular.forEach(INVOICE_DATA, function (single_display_data, key) {
        if (
          single_display_data.coupon_id != 0 &&
          single_display_data.coupon_id != undefined
        ) {
          var request_data = {
            service_id: single_display_data.data_id,
            coupon_id: single_display_data.coupon_id,
            customer_id: UNPAID_INVOICE.customer.id,
          };
          $http
            .post(APP.API + "coupon_by_id", request_data)
            .then(function (response) {
              try {
                if (response.data.status == "401") {
                  restSessionOut.getRstOut();
              ...
              if ($scope.invoice_id != 0) {
                current_coupon.hmt_used =
                  current_coupon.hmt_used - single_display_data.quantity;
                $scope.services.find(function (single_service) {
                  if (single_service.id == single_display_data.data_id) {
                    single_display_data.calculation_sale_price =
                      single_service.sales_price;
                  }
                });
                if ($scope.couponIds_hmt_adjust.length > 0) {
                  var c_index = $scope.couponIds_hmt_adjust.findIndex(function (
                    c_id
                  ) {
                    return c_id.coupon_id == current_coupon.id;
                  });
                  if (c_index == -1) {
                    $scope.couponIds_hmt_adjust.push({
                      coupon_id: current_coupon.id,
                      hmt_used: single_display_data.quantity,
                    });
                  } else {
                    $scope.couponIds_hmt_adjust[c_index].hmt_used =
                      $scope.couponIds_hmt_adjust[c_index].hmt_used +
                      single_display_data.quantity;
                  }
                } else {
                  $scope.couponIds_hmt_adjust.push({
                    coupon_id: current_coupon.id,
              ...
              angular.forEach(
                current_coupon.coupon_detail.services,
                function (service_no) {
              ...
              $scope.display_cashdesk_data.push({
                ...
                is_disabled:
                  single_display_data.is_disabled == 1 ? true : false,
                ...
                calculation_sale_price: parseFloat(
                  (
                    current_coupon.coupon_detail.sale_price /
                    $scope.total_no_of_services
                  ).toFixed(2)
                ),
    ...
    $scope.forDisplayData = function (display_data) {
      angular.forEach(display_data, function (single_display_data, key) {
        if (single_display_data.which_one != "gift") {
          if (single_display_data.which_one != "coupon") {
            var check_index = taxids_display_below_table.findIndex(function (
              check_tax_id
            ) {
      ...
      angular.forEach(group_by_taxs, function (group_by_tax, key) {
        if (key != 0) {
          var _TAX = $scope.taxes.find(function (tax) {
            return tax.id == key;
          });
          var product_or_service_tax_price = 0;
          for (var i = 0; i < group_by_tax.length; i++) {
            if (group_by_tax[i].is_check) {
              var product_or_service_price_with_tax =
                group_by_tax[i].calculation_sale_price;
              var product_or_service_price_actual_price =
                group_by_tax[i].quantity *
                (product_or_service_price_with_tax /
                  (1 + _TAX.tax_value / 100));
              product_or_service_tax_price +=
                product_or_service_price_with_tax -
                product_or_service_price_actual_price;
              // total_price += group_by_tax[i].calculation_sale_price
              total_price += parseFloat(group_by_tax[i].calculation_sale_price);
            }
          }
          $scope.group_by_taxs.push({
            taxid: key,
            total_amount_of_this_tax: product_or_service_tax_price.toFixed(2),
            tax_value: _TAX.tax_value,
          });
        } else {
          for (var i = 0; i < group_by_tax.length; i++) {
            if (group_by_tax[i].is_check) {
              ...
              if (group_by_tax[i].which_one == "product") {
                ...
                if (group_by_tax[i].quantity > group_by_tax[i].stock) {
                  ...
                  notifications.Message(
                    ...
                    "Availabe stock is " + group_by_tax[i].stock + " !!"
                  );
                } else {
                  group_by_tax[i].single_row_total =
                    group_by_tax[i].calculation_sale_price *
      ...
      if ($scope.invoice_id != 0) {
        angular.forEach($scope.table_data, function (single_data, key) {
          if (single_data.discount_apply == 3) {
            var coupon_hmt_for_adjust = $scope.couponIds_hmt_adjust.find(
              function (cpn_hmt) {
                return single_data.coupon.id == cpn_hmt.coupon_id;
              }
            );
            if (
              single_data.coupon != undefined &&
              coupon_hmt_for_adjust != undefined
            ) {
              single_data.coupon.hmt_used =
                single_data.coupon.no_of_services -
                coupon_hmt_for_adjust.hmt_used;
            }
          }
        });
      }
      angular.forEach($scope.table_data, function (single_data, key) {
        if (single_data.discount_apply == 3) {
          var current_validity =
            single_data.coupon.no_of_services - single_data.coupon.hmt_used;
          if (current_validity < single_data.quantity) {
            if (current_validity == 0) {
              $scope.total_coupon_amount = 0;
            } else {
              $scope.total_coupon_amount +=
                current_validity * single_data.calculation_sale_price;
              single_data.discount_amount =
                current_validity * single_data.calculation_sale_price;
            }
          } else if (current_validity > single_data.quantity) {
            $scope.total_coupon_amount +=
              single_data.quantity * single_data.calculation_sale_price;
            single_data.discount_amount =
              single_data.quantity * single_data.calculation_sale_price;
          } else if (current_validity == single_data.quantity) {
            $scope.total_coupon_amount +=
        ...
        if (single_data.which_one == "service") {
          var serviceKey = Object.keys($scope.services).find(
            (key) => $scope.services[key].id === single_data.id
          );
          if (serviceKey !== undefined) {
            single_data.prestatie_code =
              $scope.services[serviceKey].prestatie_code;
          }
        }
      });
    ...
    $scope.checkDiscount = function (index, single_data) {
      if (!$scope.table_data[index].is_check) {
        ...
        $scope.table_data[index].calculation_sale_price =
          $scope.table_data[index].calculation_sale_price;
        $scope.table_data[index].single_row_total =
          $scope.table_data[index].calculation_sale_price;
    ...
    $scope.clickOnDiscount = function (index, single_data) {
      if (!$scope.table_data[index].is_check) {
        notifications.Message(
          "warning",
          "Not allowed",
          "First checked item before apply discount !!"
        );
        return false;
      }
      if (single_data.discount_apply == 0) {
        $scope.table_data[index].data_before_discount = {
          calculation_sale_price: single_data.calculation_sale_price,
          discount_amount: single_data.discount_amount,
          discount_apply: single_data.discount_apply,
          discount_percentage: single_data.discount_percentage,
          display_sale_price: single_data.display_sale_price,
          id: single_data.id,
          name: single_data.name,
          quantity: single_data.quantity,
          single_row_comment: single_data.single_row_comment,
          single_row_total: single_data.single_row_total,
          tax_id: single_data.tax_id,
          which_one: single_data.which_one,
          is_check: single_data.is_check,
        };
          ...
          $scope.data_inside_modal,
          {
            success: function (state, title, msg) {
              ...
              if (DISCOUNT_DATA.type == "amount") {
                ...
                $scope.table_data[index].discount_amount =
                  DISCOUNT_DATA.value_amount;
                $scope.table_data[index].discount_percentage = 0;
                $scope.changeInTableData();
              } else if (DISCOUNT_DATA.type == "percentage") {
                $scope.table_data[index].discount_apply = 2; // 2 = percentage
                $scope.table_data[index].discount_percentage =
                  DISCOUNT_DATA.value_percentage;
                $scope.table_data[index].discount_amount = 0;
                $scope.changeInTableData();
                ...
                angular.forEach($scope.table_data, function (single_row) {
                  if (single_row.coupon != undefined) {
                    if (
                      single_row.coupon.id != undefined &&
                      single_row.discount_apply == 3
                    ) {
                      $scope.service_quantity_after_modal.push({
                        service_id: single_row.id,
                        quantity: single_row.quantity,
                      });
                    }
                  }
                });
                $scope.service_quantity_sum_after_modal = [];
                $scope.service_quantity_after_modal.reduce(function (
                  res,
                  value
                ) {
                  if (!res[value.service_id]) {
                    ...
                    $scope.service_quantity_sum_after_modal.push(
                      res[value.service_id]
                    );
                  }
                ...
                {});
                var no_of_services_used =
                  $scope.service_quantity_sum_after_modal.find(function (
                    single_row_info
                  ) {
                    return (
                      single_row_info.serviceId == $scope.table_data[index].id
                    );
                  });
                if (no_of_services_used == undefined) {
                  var no_of_service_used_till_now = 0;
                } else {
                  var no_of_service_used_till_now =
                    no_of_services_used.quantity;
                }
                var remaining_service_till_now =
                  $scope.table_data[index].coupon.no_of_services -
                  $scope.table_data[index].coupon.hmt_used;
                if (
                  $scope.table_data[index].quantity >
                  remaining_service_till_now - no_of_service_used_till_now
                ) {
                  notifications.Message(
                    "warning",
                    "Warning",
                    "Quantity is greater than remaining coupon services"
                  );
                  $scope.changeInTableData();
                } else {
                  var total_no_of_serv = 0;
                  angular.forEach(
                    DISCOUNT_DATA.coupon.coupon_detail.services,
                    function (service_no) {
                      total_no_of_serv += service_no.no_of_services;
                    }
                  );
                  ...
                  $scope.table_data[index].calculation_sale_price = parseFloat(
                    (
                      DISCOUNT_DATA.coupon.coupon_detail.sale_price /
                      $scope.total_no_of_services
                    ).toFixed(2)
                  );
        ...
        if ($scope.table_data[index].discount_apply != 3) {
          ...
          angular.forEach($scope.services, function (single_service) {
            if (
              single_service.coupon.id == undefined &&
              single_service.id == $scope.table_data[index].id
            ) {
            ...
            if (single_service.coupon != undefined) {
              if (single_service.coupon.cc_number != undefined) {
                single_service.label =
                  ...
                  single_service.sales_price +
                  ") " +
                  single_service.coupon.cc_number;
              } else {
                single_service.label =
                  single_service.name +
                  "  ( ₹" +
          ...
          $scope.table_data[index].data_before_discount = {};
          $scope.table_data[index].calculation_sale_price =
            $scope.table_data[index].display_sale_price;
          $scope.table_data[index].discount_amount = 0;
        }
        $scope.changeInTableData();
      }
    ...
    $scope.removeTableData = function (index, single_data) {
      SweetAlert.swal(
        ...
        function (isConfirm) {
          if (isConfirm) {
            if (
              $scope.table_data[index].appointment_id &&
              $scope.table_data[index].which_one == "service"
            ) {
              $scope.removedExistingServiceAppointmentIds.push(
                $scope.table_data[index].appointment_id
              );
            }
      ...
      modalProvider.openPopupModal(
        ...
        $scope.data_inside_modal,
        {
          success: function (state, title, msg) {
            ...
            $scope.table_data[index].single_row_comment =
              EDITED_ROW.single_row_comment;
          },
        }
      );
    };
    ...
    $scope.onSelectCallback = function (item) {
      ...
      if (item.id != 0) {
        ...
        $http
          .get(APP.API + "customer_coupon_get/" + cust_id)
          .then(function (response) {
            try {
              if (response.data.status == "401") {
                restSessionOut.getRstOut();
            ...
            if (CurrentDate > CheckDate) {
              ...
              $http
                .get(APP.API + "delete_customer_membership/" + cust_id)
                .then(function (response) {
                  try {
                    if (response.data.status == "401") {
                      restSessionOut.getRstOut();
                  ...
                  notifications.Message(
                    "warning",
                    $scope.customer.full_name,
                    "This Customer Service Is Expired"
                  );
                })
                .catch(function (request, status, errorThrown) {});
            } else {
        ...
        $scope.ui_select_customer =
          $scope.ui_select_customer == true ? false : true;
      } else {
        $scope.customer = item;
        $scope.ui_select_customer =
          $scope.ui_select_customer == true ? false : true;
    ...
    $scope.saveInvoice = function (which_button) {
      ...
      if ($scope.remaining_amount != undefined) {
        ...
        $scope.invoice.removedExistingServiceAppointmentIds =
          $scope.removedExistingServiceAppointmentIds;
        if (
          $scope.invoice.to_pay_id != undefined &&
          $scope.invoice.to_pay_id != 0
        ) {
          $http
            .post(APP.API + "save_invoice", $scope.invoice)
            .then(function (response) {
              try {
                if (response.data.status == "401") {
                  restSessionOut.getRstOut();
              ...
              // if ($scope.invoice_id.status == 0) {
              //   notifications.Message(
              //     "warning",
              //     "Warning",
              //     $scope.invoice_id.message
              //   );
              // }
    ...
    $scope.yourFunction = function (search) {
      ...
      $http
        .post(APP.API + "customer_for_appointment", search_data)
        .then(function (response) {
          try {
            if (response.data.status == "401") {
              restSessionOut.getRstOut();
    ...
    $scope.service_text = {
      buttonDefaultText: "Click here to select services",
    };
    $scope.service_male_text = {
      buttonDefaultText: "Click here to Male select services",
    };
    $scope.service_female_text = {
      buttonDefaultText: "Click here to female select services",
    };
            ...
            if (service.id == id) {
              if (service.coupon != undefined) {
                if (service.coupon.cc_number != undefined) {
                  var total_no_of_serv = 0;
                  angular.forEach(
                    service.coupon.coupon_detail.services,
                    function (service_no) {
                      total_no_of_serv += service_no.no_of_services;
                    }
                  );
                  ...
                  $scope.display_cashdesk_data.push({
                    ...
                    calculation_sale_price: parseFloat(
                      (
                        service.coupon.coupon_detail.sale_price /
                        $scope.total_no_of_services
                      ).toFixed(2)
                    ),
                  ...
                  $scope.display_cashdesk_data.push({
                    ...
                    data_before_discount: {},
                    discount_apply: 0,
                    single_row_total: service.sales_price,
                    single_row_comment: "",
                    worker_id: $scope.first_worker_id,
                    service_treatment_date: new Date(),
                    prestatie_code: service.prestatie_code,
    ...
    $scope.clickAddService1 = function () {
      if ($scope.select_services_female_membership.length > 0) {
        var service_ids = $scope.select_services_female_membership.map(
          ({ id }) => id
        );
        angular.forEach(service_ids, function (id, key) {
          $scope.services.find(function (service) {
              ...
              if (service.coupon != undefined) {
                if (service.coupon.cc_number != undefined) {
                  ...
                  $scope.display_cashdesk_data.push({
                    ...
                    calculation_sale_price: parseFloat(
                      (
                        service.coupon.coupon_detail.membership_price /
                        $scope.total_no_of_services
                      ).toFixed(2)
                    ),
                    tax_id: service.tax_id,
    ...
    $scope.clickMaleAddService1 = function () {
      if ($scope.select_services_male_membership.length > 0) {
        var service_ids = $scope.select_services_male_membership.map(
          ({ id }) => id
        );
        angular.forEach(service_ids, function (id, key) {
          $scope.services.find(function (service) {
      ...
      angular.forEach($scope.services, function (single_service) {
        $scope.service_quantity_sum.find(function (current_service_qty) {
          if (current_service_qty.serviceId == single_service.id) {
            var remaining_services =
              single_service.coupon.no_of_services -
              single_service.coupon.hmt_used;
        ...
        if (single_service.coupon != undefined) {
          if (single_service.coupon.cc_number != undefined) {
            ...
            single_service.label =
              single_service.name + "  ( ₹" + single_service.sales_price + ")";
          }
        } else {
          single_service.label =
            single_service.name + "  ( ₹" + single_service.sales_price + ")";
      ...
      if ($scope.select_products.length > 0) {
        ...
        if (product_index === -1) {
              ...
              if (product.id == id) {
                if (product.stocke > 0) {
                  $scope.display_cashdesk_data.push({
                    ...
                    data_before_discount: {},
                    discount_apply: 0,
                    single_row_total: product.sale_price,
                    single_row_comment: "",
                    worker_id: $scope.first_worker_id,
                    stock: product.stocke,
                  });
                } else {
                  notifications.Message(
                    "warning",
          ...
          notifications.Message(
            ...
            "You can increase quantity of selected product !!"
          );
        }
        $scope.select_products = [];
      } else {
    ...
    $scope.toPay = function () {
      if ($scope.table_data != undefined && $scope.table_data.length > 0) {
        if ($scope.invoice.customer == undefined) {
          notifications.Message("error", "Error", "Please Select Customer.");
          return false;
        }
        ...
        notifications.Message(
          "warning",
          "Warning",
          "There is no service selected for payment."
        );
      }
    };
    ...
    $scope.paying_amount_fn = function (payment) {
      ...
      if ($scope.invoice.customer != undefined) {
        angular.forEach($scope.table_data, function (single_data) {
          if (
            single_data.appointment_id == 0 &&
            single_data.which_one == "service" &&
            single_data.is_check == true
          ) {
            $scope.service_data_array.push(single_data);
          }
        });
    ...
    $scope.sendToSaveInvoice = function () {
      ...
      angular.forEach($scope.table_data, function (single_data, index) {
        if ($scope.removedExistingServiceAppointmentIds.length > 0) {
          if (
            $scope.removedExistingServiceAppointmentIds.includes(
              $rootScope.clickedAppointmentId
            )
          ) {
        ...
        $scope.table_data[index].is_disabled =
          single_data.is_check == true ? 1 : 0;
      });
    ...
    $scope.add_data_in_invoice_payment = function () {
      ...
      $http
        .post(APP.API + "save_payment", $scope.data_for_invoice_payment)
        .then(function (response) {
          try {
            if (response.data.status == "401") {
              restSessionOut.getRstOut();
          ...
          if (response.data.success == true) {
            ...
            storeAppointmentData.set({});
            $state.go("app.pages.invoice/:id/:data_id", {
              id: $scope.invoice_id,
              data_id: 0,
            });
          }
    ...
    $scope.today = function () {
      ...
      $scope.invoice.treatment_date =
        UNPAID_INVOICE.treatment_date != undefined
          ? new Date(UNPAID_INVOICE.treatment_date)
          : new Date();
    };
    $scope.today();
    ...
    $scope.toggleMin = function () {
      $scope.datepickerOptions.minDate = $scope.datepickerOptions.minDate
        ? null
        : new Date();
      $scope.dateDisabledOptions.minDate = $scope.dateDisabledOptions.minDate
        ? null
        : new Date();
    };
...
app.controller("showServicesInCashDeskCtrl", [
  "$scope",
  "$uibModalInstance",
  "$http",
  "items",
  ...
  function (
    ...
    $scope.onKeyUp = function () {
      ...
      if (
        serach_value.search_service.length > 0 ||
        serach_value.search_product != undefined
      ) {
        if (
          serach_value.search_product.length > 0 ||
          serach_value.search_service.length > 0
        ) {
      ...
      $http
        .post(APP.API + php_api_link, search_data)
        .then(function (response) {
          try {
            if (response.data.status == "401") {
              restSessionOut.getRstOut();
    ...
    $scope.checkedRadio = function (which_checked) {
      $scope.discount.type = which_checked;
    };
    $scope.AddServiceProduct = function (which_one) {
      switch (which_one) {
        case "services":
          ...
          angular.forEach(checked_service, function (service) {
            $scope.cashdesk_full_data.push({
              ...
              single_row_total: service.sales_price,
              single_row_comment: "",
              worker_id: items.first_worker_id,
            });
          });
        ...
        case "gift":
          if (
            $scope.gift_coupon.unicode == undefined ||
            $scope.gift_coupon.unicode == ""
          ) {
          ...
          var gift_card = {
            which_one: "gift",
            id: $scope.gift_coupon.unicode,
            is_check: true,
            appointment_id: 0,
            name: "Gift Certificate(" + $scope.gift_coupon.unicode + ")",
            display_sale_price: $scope.gift_coupon.sale_price,
            calculation_sale_price: $scope.gift_coupon.sale_price,
            tax_id: 0,
            quantity: 1,
            discount_amount: 0,
            discount_percentage: 0,
            data_before_discount: {},
            discount_apply: 0,
            single_row_total: $scope.gift_coupon.sale_price,
            single_row_comment: "",
            worker_id: items.first_worker_id,
          };
        ...
        case "coupons":
          ...
          // }
          if (
            $scope.entrance_date.date == null ||
            $scope.entrance_date.date == ""
          ) {
            notifications.Message("error", "Error", "Choose entrance date !!");
            return false;
          }
          if ($scope.end_date.date == null || $scope.end_date.date == "") {
            notifications.Message("error", "Error", "Choose End date !!");
            return false;
          }
          var coupon_card = {
            which_one: "Membership",
            id: $scope.checked_coupon_data.id,
            is_check: true,
            appointment_id: 0,
            name: "Membership",
            display_name: "Membership",
            display_sale_price: $scope.checked_coupon_data.sale_price,
            calculation_sale_price: $scope.checked_coupon_data.sale_price,
            tax_id: 0,
            quantity: 1,
            discount_amount: 0,
            discount_percentage: 0,
            data_before_discount: {},
            discount_apply: 0,
            single_row_total: $scope.checked_coupon_data.sale_price,
            single_row_comment: "",
            worker_id: items.first_worker_id,
            coupon_start_date: moment($scope.entrance_date.date).format(
              "YYYY-MM-DD"
            ),
            coupon_end_date: moment($scope.end_date.date).format("YYYY-MM-DD"),
          };
        ...
        case "discount":
          if ($scope.discount.type == "amount") {
            if (
              $scope.discount.value_amount >
              $scope.selected_row_for_discount.calculation_sale_price *
                $scope.selected_row_for_discount.quantity
            ) {
              notifications.Message(
                "error",
                "Error",
                "Discount amount should be less than total price !!"
              );
            ...
            if ($scope.discount.value_percentage > 99) {
              notifications.Message(
                "error",
                "Error",
                "Not allow more than 99% !!"
              );
              $scope.discount.value_percentage = 0;
              return false;
            } else if (
              $scope.discount.value_percentage == undefined ||
              $scope.discount.value_percentage == 0
            ) {
              notifications.Message(
                "error",
                "Error",
                "Discount percentage should be greater than zero !!"
              );
              $scope.discount.value_percentage = 0;
              return false;
            }
    ...
    var end_date = moment($scope.entrance_date.date, "DD-MM-YYYY").add(
      "month",
      12
    );
    $scope.end_date = { date: end_date._d };
    ...
    $scope.dateChanged_endDate = function () {
      var check1 = moment(
        moment($scope.end_date.date, "DD-MM-YYYY").format("YYYY-MM-DD")
      ).isBefore(moment(new Date(), "DD-MM-YYYY").format("YYYY-MM-DD"));
      if (check1) {
        notifications.Message(
          "warning",
          "Not allowed past date !!",
          "End Date should be after or equal to today."
        );
        $scope.end_date = { date: "" };
      } else {
        // var end_date = moment($scope.entrance_date.date, 'DD-MM-YYYY').add('month', $scope.checked_coupon_data.validity);
...
app.controller("showToPayCashDeskCtrl", [
  "$scope",
  "$uibModalInstance",
  "$http",
  "items",
  ...
  function (
    ...
    $scope.$watch("received_amount", function () {
      if (
        parseFloat($scope.received_amount) != parseFloat($scope.paid_amount)
      ) {
    ...
    $scope.getPaidAmount = function () {
      $http
        .get(APP.API + "get_payment_by_invoice_id/" + $scope.invoice_id)
        .then(function (response) {
          try {
            if (response.data.status == "401") {
              restSessionOut.getRstOut();
    ...
    $scope.payMethod = function (index, _coming_method) {
      if (_coming_method.method == "gift_certificate") {
        ...
        $scope.table_data[index].status =
          $scope.table_data[index].status == false ? true : false;
        ...
        angular.forEach($scope.table_data, function (single_data, key) {
          ...
          if (
            $scope.paid_amount == $scope.received_amount ||
            $scope.paid_amount == 0
          ) {
            $scope.flag = 1;
            if (key == 4) {
    ...
    $scope.amountBoxBlur = function () {
      ...
      angular.forEach($scope.table_data, function (single_data, key) {
        if (keepGoing) {
          ...
          single_data.status =
            single_data.amount == 0 || single_data.amount == null
              ? false
              : true;
          if (single_data.amount != undefined) {
            if (single_data.amount > $scope.paid_amount) {
              ...
              notifications.Message(
                "warning",
                "Enter valid amount",
                "Received amount is greater than paid amount !!"
              );
            } else {
      ...
      if (
        $scope.received_amount == $scope.paid_amount ||
        $scope.paid_amount == 0
      ) {
        $scope.flag = 1;
        angular.forEach($scope.table_data, function (single_data, key) {
    ...
    $scope.after_add_gift = function () {
      ...
      if (gifted_amount >= $scope.paid_amount) {
        $scope.table_data[4].amount =
          $scope.paid_amount - total_received_without_gift;
        angular.forEach($scope.table_data, function (single_data, key) {
        ...
        var gifted_plus_witout_gifted =
          gifted_amount + total_received_without_gift;
    ...
    $scope.removeGift = function (gift) {
      ...
      var gift_index = $scope.gift_received.findIndex(function (single_gift) {
        return single_gift.id == gift.id;
      });
      if (gift_index > -1) {
      ...
      if (gifted_amount >= $scope.paid_amount) {
        ...
        if (gifted_plus_witout_gifted >= $scope.paid_amount) {
          $scope.table_data[4].amount =
            gifted_plus_witout_gifted - $scope.paid_amount;
          angular.forEach($scope.table_data, function (single_data, key) {
            if (received_amount_with_gift != $scope.paid_amount) {
              single_data.display_icon = single_data.amount == 0 ? false : true;
            } else {
    ...
    $scope.completePayment = function () {
      ...
      $http
        .post(APP.API + "deduct_product_quantity", products)
        .then(function (response) {})
        .catch(function (request, status, errorThrown) {});
      $scope.payment = {
        invoice_id: $scope.invoice_id,
    ...
    $scope.completePayment1 = function () {
      ...
      if ($scope.paid_amount == total_amount) {
        $http
          .post(APP.API + "save_payment", $scope.payment)
          .then(function (response) {
            try {
              if (response.data.status == "401") {
                restSessionOut.getRstOut();
...
app.controller("giftListCtrl", [
  ...
  "cb",
  "restSessionOut",
  function ($scope, $uibModalInstance, $http, items, APP, cb, restSessionOut) {
    var auth_token = localStorage.getItem("auth_token");
    $http.defaults.headers.common.authtoken = auth_token;
    if (!localStorage.getItem("auth_token")) {
      restSessionOut.getRstOut();
      ...
      .then(function (response) {
        try {
          ...
          angular.forEach(items.selected_gift_ids, function (value, key) {
            var index = GIFTS.findIndex(function (gift) {
...
app.controller("askSaveAppointmentCtrl", [
  "$scope",
  "$uibModalInstance",
  "$http",
  "items",
  ...
  function (
    ...
    $scope.saveAppointment = function () {
      $scope.appointment.start_time =
        ($scope.appointment_date.getHours().toString().length > 1
          ? $scope.appointment_date.getHours().toString()
          : "0" + $scope.appointment_date.getHours().toString()) +
        ":" +
        ($scope.appointment_date.getMinutes().toString().length > 1
          ? $scope.appointment_date.getMinutes().toString()
          : "0" + $scope.appointment_date.getMinutes().toString());
      $scope.appointment.appointment_date = $scope.appointment_date;
      $scope.appointment.paid_appointment = "paid";
      $http
        .post(APP.API + "add_appointment", $scope.appointment)
        .then(function (response) {
          try {
            if (response.data.status == "401") {
              restSessionOut.getRstOut();
...
app.controller("createCouponCtrl", [
  "$scope",
  "$uibModalInstance",
  "$http",
  "items",
  ...
  function (
    ...
    $scope.service_list_for_coupon = function () {
      var service_data = {
        ...
        tax_id:
          $scope.current_tax_id == undefined ? 0 : $scope.current_tax_id.id,
      };

      $http
        .post(APP.API + "service_list_for_coupon", service_data)
        .then(function (response) {
          try {
            if (response.data.status == "401") {
              restSessionOut.getRstOut();
    ...
    $scope.showServiceModal = function () {
      $scope.data_inside_modal = {
        ...
        service_form_id:
          $scope.service_ids_for_form == undefined
            ? []
            : $scope.service_ids_for_form,
        tax__id:
          $scope.current_tax_id == undefined ? 0 : $scope.current_tax_id.id,
      };
    ...
    $scope.removeServiceFrom_Form = function (serviceid) {
      ...
      if (services_array.length != 0 && $scope.service_for_form.length) {
        var find_index_of_id = service_ids.findIndex(function (
          single_service_id
        ) {
        ...
        var find_index_for_remove = services_array.findIndex(function (
          single_service
        ) {
    ...
    $scope.saveCouponForm = function () {
      var php_api =
        $scope.coupon.id != undefined && $scope.coupon.id != null
          ? "update_coupon"
          : "add_coupon";
      if ($scope.coupon.services == undefined) {
        notifications.Message(
      ...
      if (
        $scope.coupon.validity == undefined ||
        $scope.coupon.validity == null
      ) {
      ...
      if ($("#id_couponForm").valid()) {
        if (
          !$scope.sale_price_validation_message_display &&
          !$scope.validity_validation_message_display
        ) {
          ...
          var coupon_card = {
            ...
            is_check: true,
            appointment_id: 0,
            name: $scope.coupon.description,
            display_name: "Membership",
            display_sale_price: parseFloat($scope.coupon.sale_price),
            calculation_sale_price: parseFloat($scope.coupon.sale_price),
            tax_id: 0,
            quantity: 1,
            discount_amount: 0,
            discount_percentage: 0,
            data_before_discount: {},
            discount_apply: 0,
            single_row_total: $scope.coupon.sale_price,
            single_row_comment: "",
            worker_id: items.first_worker_id,
            coupon_start_date: moment($scope.entrance_date.date).format(
              "YYYY-MM-DD"
    ...
    $scope.dateChanged = function () {
      var check1 = moment(
        moment($scope.entrance_date.date, "DD-MM-YYYY").format("YYYY-MM-DD")
      ).isBefore(moment(new Date(), "DD-MM-YYYY").format("YYYY-MM-DD"));
      if (check1) {
        notifications.Message(
          "warning",
          "Not allowed past date !!",
          "Date should be after or equal to today."
        );
        $scope.entrance_date = { date: "" };
        $scope.end_date = "";
      } else {
        var end_date = moment($scope.entrance_date.date, "DD-MM-YYYY").add(
          "month",
          $scope.checked_coupon_data.validity
        );
        $scope.end_date = end_date._d;
      }
    };
...
app.controller("replaceServiceModalCtrl", [
  "$scope",
  "$uibModalInstance",
  "$http",
  "items",
  ...
  function (
    ...
    $scope.appointment_id =
      $scope.display_cashdesk_data[$scope.index].appointment_id;
    $scope.cancel = function () {
      $uibModalInstance.close();
    };
    $scope.replace = function () {
      ...
      if ($scope.select_services.length > 0) {
        ...
        const alreadyExists = $scope.display_cashdesk_data.some(
          (el) => el.id == $scope.selectedService
        );
        if (alreadyExists) {
          notifications.Message(
            "warning",
            "Warning",
            "Service already added please select another!"
          );
          return false;
        }
      ...
      $http
        .post(APP.API + "update_service_from_topay", request_data)
        .then(function (response) {
