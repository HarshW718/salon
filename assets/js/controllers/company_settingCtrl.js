app.controller("company_settingCtrl", [
  "$scope",
  "restSessionOut",
  "$http",
  "APP",
  ...
  function (
    ...
    $http.get(APP.API + "company_details").then(function (response) {
      if (response.data.success == false) {
        $scope.company = response.data.data;
        debugger;
        console.log(Number($scope.company.mobile));

        $scope.company.mobile = Nunber(response.data.data.mobile);
      }
    });
    ...
    $scope.updateCompanyDetails = function () {
      if ($("#id_companyForm").valid()) {
        if (
          $scope.imageError == 1 ||
          $scope.company.company_logo == undefined ||
        ...
        $http
          .post(APP.API + "update_company_details", $scope.company)
    ...
    $scope.moveToTrash = function () {
      SweetAlert.swal(
        ...
        function (isConfirm) {
          if (isConfirm) {
            $http
              .post(APP.API + "delete_company_images", $scope.company)
              .then(function (response) {
                try {
    ...
    uploaderImages.onSuccessItem = function (
      fileItem,
      response,
      status,
      headers
      ...
      if (response == "error") {
        ...
        notifications.Message(
          "error",
          "Error",
          "Your image was larger than 2MB in file size."
        );
      } else {
        $scope.imageError = 0;
    ...
    uploaderImages.onErrorItem = function (
      fileItem,
      response,
      status,
      headers
    ) {};
    uploaderImages.onCancelItem = function (
      fileItem,
      response,
      status,
      headers
    ) {};
    uploaderImages.onCompleteItem = function (
      fileItem,
      response,
      status,
      headers
