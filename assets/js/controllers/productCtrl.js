...
app.controller("productCtrl", [
  ...
  function (
    ...
    $scope.list_product = function () {
      $http
        ...
        .then(function (response) {
          ...
          var product_table_header =
            "<table><tr><th>Category</th><th>Type</th><th>EAN</th><th>SKU</th><th>Product</th><th>Purchase</th><th>Sale</th><th>BTW</th><th>Supplier</th><th>Stock</th><th>Minimal</th><th>Shortage</th></tr>";
          ...
          angular.forEach(response.data.data, function (product, key) {
            ...
            var _tax =
              product.product_tax == null ? "---" : product.product_tax.name;
            product_table_tr +=
              "<tr><td>" +
              ...
              product.min_stocke +
              "</td><td>" +
              (product.stocke - product.min_stocke) +
              "</td></tr>";
          });

          //$scope.downloadCsv = productsCsv;
          $scope.html_data =
            product_table_header + product_table_tr + product_table_footer;
    ...
    $scope.deleteProduct = function (product_id) {
      ...
      SweetAlert.swal(
        ...
        function (isConfirm) {
          if (isConfirm) {
            $http
              .post(APP.API + "delete_product", $scope.delete_product_id)
              .then(function (response) {
                try {
                  if (response.data.status == "401") {
                    restSessionOut.getRstOut();
    ...
    $scope.moveToTrash = function () {
      ...
      if (checkArr.length == 0) {
        notifications.Message(
          "error",
          "Error",
          "Please select product for delete"
        );
        return false;
      }
      SweetAlert.swal(
        ...
        function (isConfirm) {
          if (isConfirm) {
            $http
              .post(APP.API + "delete_product_all", $scope.delete_ids)
              .then(function (response) {
                try {
                  if (response.data.status == "401") {
                    restSessionOut.getRstOut();
                ...
                notifications.Message(
                  "success",
                  "Success",
                  "Selected product delete successfully !!"
                );
                $scope.dtInstanceInvoiceRequest.rerender();
              })
              .catch(function (request, status, errorThrown) {});
    ...
    var titleHtml =
      '<div class="checkbox clip-check check-primary ng-scope"><input type="checkbox" id="0_ischeck_0" class="select_all" ng-click="clickme()"><label for="0_ischeck_0"></label></div>';
    $scope.dtOptionInvoiceRequest_main = DTOptionsBuilder.newOptions()
        ...
        $http({
          ...
          function (response) {
            ...
            if (response.data.success) {
              ...
              $("#productDatatable_paginate")
                .parent()
                .addClass("pagination_width");
            } else {
            }
          },
          function (error) {}
    ...
    $scope.dtColumnInvoiceRequest_main = [
      ...
      DTColumnBuilder.newColumn("category_name")
        .withTitle("Category")
        .withOption("sName", "category_name"),
      DTColumnBuilder.newColumn("company_name")
        .withTitle("Supplier")
        .withOption("sName", "company_name"),
      ...
      DTColumnBuilder.newColumn("stocke")
        .withTitle("Stock")
        .withOption("sName", "stocke")
        .renderWith(stockelist),
      DTColumnBuilder.newColumn("min_stocke")
        .withTitle("Minimum")
        .withOption("sName", "min_stocke"),

      DTColumnBuilder.newColumn("shortage")
        .withTitle("Shortage")
        .withOption("sName", "shortage")
        .renderWith(shortagelist),
      DTColumnBuilder.newColumn("sale_price")
        .withTitle("Sale Price")
        .withOption("sName", "sale_price"),
      DTColumnBuilder.newColumn("purchase_price")
        .withTitle("Purchase Price")
        .withOption("sName", "purchase_price"),
      DTColumnBuilder.newColumn("id")
        .withTitle("Action1")
        .withOption("sName", "id")
        .renderWith(actionList),
    ];
    ...
    function checkboxRow(data, type, full, meta) {
      return (
        '<div class="checkbox clip-check check-primary ng-scope"><input type="checkbox" id="' +
        data +
        '_ischeck_0" class="checkBoxClass" ng-click="singleCheckBoxClick()" name="check" value="' +
        data +
        '" />  <label for="' +
        data +
        '_ischeck_0"></label></div>'
      );
    }
    function displayOngoningCheckbox(data, type, full, meta) {
      ...
      return (
        ...
        full.id +
        '_checkbox" ng-change="singleCheck(' +
        full.id +
        ')" class="ongoingCheckbox" ng-model="selected[' +
        full.id +
        ']" value="' +
        full.id +
        '">'
    ...
    function actionList(data, type, full, meta) {
      ...
      return (
        "<a ng-click='add_editProduct(" +
        full_string +
        ")' title='Edit product' ><i class='fa fa-edit text-large'></i></a><a class='margin-left-10' title='Delete product'><i ng-click='deleteProduct(" +
        full.id +
        ")' class='fa fa-remove text-large'></i></a>"
      );
    }
    ...
    $scope.dtInstanceInvoiceRequest = {};
    $scope.flag_request = 0;
    $scope.dtOptionInvoiceRequest_mobile = DTOptionsBuilder.newOptions()
      .withOption("ajax", function (data, callback, settings) {
        if (data.order[0].column == 0) {
          var sque = "desc";
          var columnmna = "id";
  ...
  function (
    ...
    $scope.submitProductForm = function () {
      if ($("#id_productForm").valid()) {
        console.log("TEST HSN CODE : ", $scope.product);
        php_api_link =
          typeof $scope.product.id !== "undefined"
            ? "update_product"
            : "add_product";
        console.log("TEST HSN CODE php_api_link : ", php_api_link);

        $http
          .post(APP.API + php_api_link, $scope.product)
          .then(function (response) {
            try {
              if (response.data.status == "401") {
                restSessionOut.getRstOut();
...
app.controller("productsCategoriesCtrl", [
  "$scope",
  "$http",
  "SweetAlert",
  "APP",
  ...
  function (
    ...
    $scope.submitProductCategoryForm = function () {
      if ($("#id_productCategoryForm").valid()) {
        ...
        php_api_link =
          typeof $scope.product_category.id !== "undefined"
            ? "update_product_category"
            : "add_product_category";
        $http
          .post(APP.API + php_api_link, $scope.product_category)
          .then(function (response) {
            try {
              if (response.data.status == "401") {
                restSessionOut.getRstOut();
      ...
      SweetAlert.swal(
        ...
        function (isConfirm) {
          if (isConfirm) {
            $http
              .post(
                APP.API + "delete_product_category",
                $scope.delete_product_category_id
              )
              .then(function (response) {
                try {
                  if (response.data.status == "401") {
    ...
    $scope.dtOptionInvoiceRequest = DTOptionsBuilder.newOptions()
      .withOption("ajax", function (data, callback, settings) {
        if (data.order[0].column == 0) {
          var sque = "desc";
          var columnmna = "id";
    ...
    function actionList(data, type, full, meta) {
      var full_string = JSON.stringify(full);
      return (
        "<a ng-click='editProductCategory(" +
        full_string +
        ")'><i class='fa fa-edit text-large'></i></a><a class='margin-left-10'><i ng-click='deleteProductCategory(" +
        full.id +
        ")' class='fa fa-remove text-large'></i></a>"
      );
    }
    function nameHtml(data, type, full, meta) {
      var full_string = JSON.stringify(full);
      return (
        "<span><i class='fa fa-list-alt text-large'></i> " +
        full.name +
        " </span>"
      );
    }
...
app.controller("productsCompaniesCtrl", [
  "$scope",
  "$http",
  "SweetAlert",
  "APP",
  ...
  function (
    ...
    $scope.submitProductCompanyForm = function () {
      if ($("#id_productCompanyForm").valid()) {
        php_api_link =
          typeof $scope.product_company.id !== "undefined"
            ? "update_company"
            : "add_company";
        $http
          .post(APP.API + php_api_link, $scope.product_company)
          .then(function (response) {
            try {
              if (response.data.status == "401") {
                restSessionOut.getRstOut();
      ...
      SweetAlert.swal(
        ...
        function (isConfirm) {
          if (isConfirm) {
            $http
              .post(
                APP.API + "delete_company",
                $scope.delete_product_company_id
              )
              .then(function (response) {
                try {
                  if (response.data.status == "401") {
    ...
    function actionList(data, type, full, meta) {
      var full_string = JSON.stringify(full);
      return (
        "<a ng-click='editProductCompany(" +
        full_string +
        ")'><i class='fa fa-edit text-large'></i></a><a class='margin-left-10'><i ng-click='deleteProductCompany(" +
        full.id +
        ")' class='fa fa-remove text-large'></i></a>"
      );
    }
    ...
    function nameHtml(data, type, full, meta) {
      var full_string = JSON.stringify(full);
      return (
        "<span><i class='fa fa-building-o text-large'></i> " +
        full.name +
        " </span>"
      );
    }
...
app.controller("uploadProductController", [
  "$scope",
  "$uibModalInstance",
  "$http",
  "items",
  ...
  function ($scope, $uibModalInstance, $http, items, APP, cb, restSessionOut) {
    ...
    $scope.submitCsvData = function () {
      $http
        .post(APP.API + "upload_csv_data", $scope.csvData)
        .then(function (response) {
          try {
            if (response.data.status == "401") {
              restSessionOut.getRstOut();
