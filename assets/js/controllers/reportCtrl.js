...
app.controller("invoiceReportCtrl", [
  "$scope",
  "$http",
  "$filter",
  "APP",
  ...
  function (
    ...
    $scope.downloadXls = function () {
      var blob = new Blob(
        [document.getElementById("invoiceReportexportable").innerHTML],
        {
    ...
    $scope.allInvoice = function (which_one) {
      $scope.filterForm.startDate_filter = $filter("date")(
        $scope.filterForm.startDate,
        "yyyy-MM-dd"
      );
      $scope.filterForm.endDate_filter = $filter("date")(
        $scope.filterForm.endDate,
        "yyyy-MM-dd"
      );
      $scope.filterForm.which_one = which_one;
      $http
        .post(APP.API + "invoice_report", $scope.filterForm)
    ...
    $scope.deletePDF_file = function (pdf_file_name) {
      $http
        .get(APP.API + "invoice_pdf_delete/" + pdf_file_name)
        .then(function (response) {
    ...
    $http
      .post(APP.API + "all_payment_methods")
      .then(function (response) {
        try {
      ...
      if (date_diff > 0) {
        ...
        if (date_diff < -366) {
          notifications.Message(
            "warning",
            "Recheck Dates !!",
            "Report will not generate more than one year !!"
          );
        } else {
          if ($("#" + frmId).valid()) {
            $scope.psDate = $filter("date")(
              $scope.filterForm.startDate,
              "yyyy-MM-dd"
            );
            $scope.peDate = $filter("date")(
              $scope.filterForm.endDate,
              "yyyy-MM-dd"
            );
    ...
    $scope.changeStartDate = function () {
      ...
      $scope.endDateOptions = {
        ...
        datepickerMode: "day",
        minDate: new Date(
          $scope.filterForm.startDate.getFullYear(),
          $scope.filterForm.startDate.getMonth(),
          $scope.filterForm.startDate.getDate()
        ),
      };
    };
    ...
    $scope.printPdf = function (div) {
      ...
      popupWin.document.write(
        '<html><head><link rel="stylesheet" type="text/css" href="' +
          path +
          'bower_components/bootstrap/dist/css/bootstrap.min.css"/><link rel="stylesheet" type="text/css" href="' +
          path +
          'assets/css/styles.css"/><style type="text/css">table{width:100% !important}</style></head><body onload="window.print()">' +
          printContents +
          "</body></html>"
      );
...
app.controller("employeeReportCtrl", [
  "$scope",
  "$http",
  "$filter",
  "APP",
  ...
  function (
    ...
    $scope.allInvoice = function (which_one) {
      ...
      $scope.filterForm.which_one = which_one;
      $http
        .post(APP.API + "employee_report", $scope.filterForm)
        .then(function (response) {
          if (response.data.pdf == 0) {
            $scope.invoiceList = response.data.invoiceList;
            $scope.downloadInvoiceCsv = allInvoiceCsv;
    ...
    console.log("$scope.first_staff_id", $scope.first_staff_id);
...
app.controller("totalReportCtrl", [
  "$scope",
  "$http",
  "$filter",
  "APP",
  ...
  function (
    ...
    $scope.downloadXls = function () {
      var blob = new Blob(
        [document.getElementById("totalReportCtrlexportable").innerHTML],
        {
          type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;charset=utf-8",
        }
      );
    ...
    $scope.totalReport = function (which_one) {
      $scope.totalReportForm.startDate_filter = $filter("date")(
        $scope.totalReportForm.startDate,
        "yyyy-MM-dd"
      );
      $scope.totalReportForm.endDate_filter = $filter("date")(
        $scope.totalReportForm.endDate,
        "yyyy-MM-dd"
      );
      $scope.totalReportForm.which_one = which_one;
      $http
        .post(APP.API + "total_report", $scope.totalReportForm)
        .then(function (response) {
          if (response.data.pdf == 0) {
          ...
          $scope.deletePDF_file = function (pdf_file_name) {
            $http
              .get(APP.API + "total_pdf_delete/" + pdf_file_name)
              .then(function (response) {
                console.log("response", response);
              })
              .catch(function (request, status, errorThrown) {});
    ...
    $scope.changeWeek = function () {
      if ($scope.selectedWeek) {
        ...
        $scope.totalReportForm.endDate = new Date(
          edate[2],
          edate[0] - 1,
          edate[1]
        );
    ...
    $scope.changeStartDate = function () {
      ...
      $scope.endDateOptions = {
        ...
        datepickerMode: "day",
        minDate: new Date(
          $scope.totalReportForm.startDate.getFullYear(),
          $scope.totalReportForm.startDate.getMonth(),
          $scope.totalReportForm.startDate.getDate()
        ),
      };
    };
    $scope.changeStartDate();
...
app.controller("salesReportCtrl", [
  "$scope",
  "$http",
  "$filter",
  "APP",
  ...
  function (
    ...
    $scope.downloadXls = function () {
      var blob = new Blob(
        [document.getElementById("salesReportexportable").innerHTML],
        {
          type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;charset=utf-8",
        }
      );
    ...
    $scope.salesReport = function (which_one) {
      $scope.salesFilterForm.startDate_filter = $filter("date")(
        $scope.salesFilterForm.startDate,
        "yyyy-MM-dd"
      );
      $scope.salesFilterForm.endDate_filter = $filter("date")(
      ...
      $http
        .post(APP.API + "sales_report", $scope.salesFilterForm)
        .then(function (response) {
          if (response.data.pdf == 0) {
    ...
    $scope.deletePDF_file = function (pdf_file_name) {
      $http
        .get(APP.API + "sales_pdf_delete/" + pdf_file_name)
        .then(function (response) {
          console.log("response", response);
        })
        .catch(function (request, status, errorThrown) {});
    ...
    $scope.changeStartDate = function () {
      $scope.endDateOptions = {
        ...
        datepickerMode: "day",
        minDate: new Date(
          $scope.salesFilterForm.startDate.getFullYear(),
          $scope.salesFilterForm.startDate.getMonth(),
          $scope.salesFilterForm.startDate.getDate()
        ),
      };
    };
    $scope.changeStartDate();
    ...
    $scope.changeWeek = function () {
      if ($scope.selectedWeek) {
        ...
        $scope.salesFilterForm.endDate = new Date(
          edate[2],
          edate[0] - 1,
          edate[1]
        );
    ...
    $scope.printPdf = function (div) {
      ...
      popupWin.document.write(
        '<html><head><link rel="stylesheet" type="text/css" href="' +
          path +
          'bower_components/bootstrap/dist/css/bootstrap.css"/><link rel="stylesheet" type="text/css" href="' +
          path +
          'assets/css/styles.css"/><link rel="stylesheet" type="text/css" href="' +
          path +
          'assets/css/custome.css"/><style type="text/css">table{width:100% !important}.services>tbody>tr>td.myCustomClass{text-align:center !important;}.services>tbody>tr>td.myCustomClass1{text-align:center !important;}</style></head><body onload="window.print()">' +
          printContents +
          "</body></html>"
      );
      popupWin.document.close();
