  ...
  function (
    ...
    $stateProvider
      ...
      .state("app.pages.services_categories", {
        ...
        templateUrl:
          "assets/views/services_categories/services_categories_list.html",
        ncyBreadcrumb: {
          label: "Categories",
        },
        resolve: loadSequence(
          "servicesCategoriesCtrl",
          "selectjs",
          "spectrum-plugin",
      ...
      .state("app.pages.employee_report", {
        url: "/employee_report",
        templateUrl: "assets/views/report/employee-report.html",
        ncyBreadcrumb: {
          label: "Report",
        },
...
app.run(function (
  ...
  $rootScope.$on("$viewContentLoaded", function () {
    if ($cookies.get("userdata") !== undefined) {
      ...
      let accountant_menu = [
        "#!/app/pages/cashdesk",
        "#!/app/pages/cash_in_out",
        "#!/app/pages/cashdesk/settings",
        "#!/app/pages/bank_transfer_invoice",
        "#!/app/pages/company_setting",
        "#!/app/pages/invoice/:id/:data_id",
      ];
      ...
      if ($cookies.get("userdata") !== undefined) {
        if (location) {
          ...
          let index = user_menu.findIndex(
            (menu) => menu.menu_url === match_path.concat(path)
          );
          ...
          if (user_menu[index]) {
            ...
            if (userdata.role_id === 4) {
              if (location[3] === "invoice") {
                ...
                reception_menu.indexOf(match_path.concat(path)) === -1
              ) {
                $timeout(function () {
                  $state.go("app.pages.calendar", null, { reload: true });
                });
            ...
            if (userdata.role_id === 3) {
              if (location[3] === "invoice") {
                ...
                accountant_menu.indexOf(match_path.concat(path)) === -1
              ) {
                $timeout(function () {
                  $state.go("app.pages.calendar", null, { reload: true });
                });
