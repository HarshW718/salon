...
class Common extends Model
{
    ...
    public static function checkFcmToken($fcmData)
    {
        ...
        $fcmData = DB::table('bet_fcm_token')->select('*')->where('fcm_token', $fcm_token)->first();
        $userFcmData = DB::table('bet_fcm_token')->select('*')->where('user_id', $user_id)->where('fcm_token', $fcm_token)->first();
        ...
        foreach ($replaceParams as $parms => $value) {
            if (strpos($body, $parms) !== false) {
                if ($parms == '[[company-logo]]') {
                    $body = str_replace($parms, '<img width="40%" src="' . $replaceParams[$parms] . '">', $body);
                } else {
